...
const Graph = (props) => {
  ...
  const loadPrevPoints = (canvas, canvasCtx) => {
    for (let entry of props.entries) {
      if (entry.r === props.rCurrent) {
        canvasCtx.fillStyle = entry.result ? 'black' : 'red';
        canvasCtx.beginPath();
        canvasCtx.arc(
  ...
  return (
    <div styleName="graph-container">
      ...
      <Canvas canvasRef={pointsCanvasRef} />
      <Canvas canvasRef={currentCanvasRef} handleClick={handleClick} />
    </div>
  );
}
